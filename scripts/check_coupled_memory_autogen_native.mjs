...
export function checkAutoGenNative(result) {
  const failures = [];
  ...
  expect(result.benchmark === "coupled-memory-phase5-autogen-native-suite", "unexpected benchmark id");
  expect(result.live === false, "native AutoGen adapter suite must be deterministic and API-free");
  expect(result.adapter_status === "native-sdk", "adapter status should be native-sdk");
  expect(result.harnesses?.[0]?.harness_id === "autogen-native", "missing autogen-native harness id");
  expect(result.harnesses?.[0]?.package === "autogen-agentchat", "missing AutoGen package metadata");
  expect(result.autogen_metadata?.autogen_agentchat, "missing autogen-agentchat version");
  expect(result.autogen_metadata?.autogen_core, "missing autogen-core version");
  expect(result.c4_c5_parity?.differing_feature === "messageBoundMemory", "C4/C5 parity metadata missing or invalid");
  expect((result.scenarios?.length ?? 0) === 27, "native AutoGen should run the full 9x3 dev split");
  ...
  return {
    ...
    checked: {
      ...
      autogen_agentchat: result.autogen_metadata?.autogen_agentchat,
      autogen_core: result.autogen_metadata?.autogen_core,
...
function markdownReport(check, input) {
  const lines = [];
  lines.push("# Coupled-Memory Native AutoGen Adapter Check");
  lines.push("");
  ...
  lines.push(`- autogen-agentchat: \`${check.checked.autogen_agentchat}\``);
  lines.push(`- autogen-core: \`${check.checked.autogen_core}\``);
...
if (import.meta.url === pathToFileURL(process.argv[1]).href) {
  const parsed = parseArgs({
    options: {
      result: { type: "string", default: "results/coupled-memory-phase5-autogen-native.json" },
      json: { type: "string", default: "results/coupled-memory-phase5-autogen-native-check.json" },
      md: { type: "string", default: "results/coupled-memory-phase5-autogen-native-check.md" },
    },
  });
